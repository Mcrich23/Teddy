...
final class CaptureService {
    ...
    /// A Boolean value that denotes if the configuration is locked
    ...
    private func setZoomControls(for device: AVCaptureDevice) throws {
        if device.position == .back, device.deviceType != .builtInWideAngleCamera {
            ...
            let factors: [ZoomFactor] = zoomFactors.map { ZoomFactor($0 / mainZoomFactor) }
            
            self.zoomFactors = factors.sorted()
        } else {
            self.zoomFactors = [1]
        }
        
//        if currentZoom > zoomFactors.maxZoom {
//            try? setZoomFactor(zoomFactors.maxZoom, device: device)
//        } else if currentZoom < zoomFactors.minZoom {
//            try? setZoomFactor(zoomFactors.minZoom, device: device)
//        } else {
...
//        }
    ...
    func setFlashMode(_ flashMode: FlashMode) throws {
        guard flashMode != self.flashMode else { return }
        self.flashMode = flashMode
        self.photoCapture.flashMode = flashMode
    }
    
    // MARK: - Zoom factor selection
    func setZoomFactor(_ zoom: ZoomFactor, device: AVCaptureDevice? = nil, animatedRate: Float?) throws {
        let device = device ?? currentDevice // Use current device if not otherwise specified
    ...
    func startPrecisionZooming() throws {
        try currentDevice.lockForConfiguration()
        isConfigurationLocked = true
    }
    
    func stopPrecisionZooming() {
